...
export class AddMembersPage {
  ...
  ionViewDidLoad() {
    ...
    // Obtenez des informations sur l'utilisateur à partir du message système et qui a été envoyé au groupe lorsqu'un membre a été ajouté.
    this.dataProvider.getCurrentUser().subscribe((user) => {
      this.user = user;
    });

    // Avoir les informations du groupe
    this.dataProvider.getGroup(this.groupId).subscribe((group) => {
      this.group = group;
      this.groupMembers = null;
      // Avoir les membres du groupe
      if (group.members) {
        ...
        // Avoir les amis de l'utilisateur pour ajouter
        this.dataProvider.getCurrentUser().subscribe((account) => {
          if (account.friends) {
            for (var i = 0; i < account.friends.length; i++) {
              this.dataProvider.getUser(account.friends[i]).subscribe((friend) => {
                // Seulement les amis qui ne sont pas déjà un membre du groupe peuvent être ajouté.
...
// Vérifiez si un ami est un membre du groupe ou non.
  isMember(friend) {
  ...
  // Vérifiez si l'ami est déjà sur la liste des membres à ajouter.
  isAdded(friend) {
    if (this.toAdd) {
